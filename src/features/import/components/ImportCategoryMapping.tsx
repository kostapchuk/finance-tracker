...
interface ImportCategoryMappingProps {
  ...
  onMappingChange: (budgetOkName: string, categoryId: string | null) => void
  onNext: () => void
  onBack: () => void
}
...
export function ImportCategoryMapping({
  ...
  // Skip step if no categories to map
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        <div className="space-y-1">
          <h2 className="text-lg font-semibold">{t('importMapCategories')}</h2>
          <p className="text-sm text-muted-foreground">{t('importMapCategoriesHint')}</p>
        </div>
