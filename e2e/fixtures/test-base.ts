...
export const test = base.extend<TestFixtures>({
  ...
  cleanupAllData: async ({ page, dbHelper }, use) => {
    const cleanup = async () => {
      // Clear local IndexedDB only - each test has unique user ID so cloud data is isolated
  ...
  setupCleanState: async ({ page, dbHelper }, use) => {
    const setup = async () => {
      ...
      await page.addInitScript(() => {
        ...
        // Set migration as complete to skip dialog
      ...
      // 7. Dismiss migration dialog if visible (click Skip)
